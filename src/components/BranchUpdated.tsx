...
import { BranchIProps, BranchIUpdatedProps } from "@/types"
...
function BranchUpdated({ data }: { data: BranchIProps }) {
	...
	// console.log(team, branch, "upload");
	...
	const form = useForm<z.infer<typeof formSchema>>({
		...
		defaultValues: {
			...
			teamLeaderOccupation: data.teamLeaderOccupation,
			teamLeaderAddress: data.teamLeaderAddress,
			presidentName: data.presidentName,
			presidentAddress: data.presidentAddress,
			presidentPhone: data.presidentPhone,
			presidentOccupation: data.presidentOccupation,
			ImamName: data.ImamName,
			ImamAddress: data.ImamAddress,
			ImamOccupation: data.ImamOccupation,
			ImamPhone: data.ImamPhone,
			SecretaryName: data.SecretaryName,
			SecretaryAddress: data.SecretaryAddress,
			SecretaryPhone: data.SecretaryPhone,
			SecretaryOccupation: data.SecretaryOccupation
		},
	});

	const { mutate, isPending } = useMutation({
		mutationFn: async ({ password, branchName, address, photoUrl, teamLeaderName, teamLeaderAddress, teamLeaderPhone, teamLeaderOccupation, teamLeaderPhotoUrl, presidentName, presidentAddress, presidentPhone, presidentOccupation, ImamName, ImamAddress, ImamPhone, ImamOccupation, SecretaryName, SecretaryAddress, SecretaryPhone, SecretaryOccupation, district, ps }: BranchIUpdatedProps) => {
			const response = await axios.patch(`/api/branch/${data.username}`, {
