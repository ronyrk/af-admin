...
async function getDetails(id: string) {
    const data = await prisma.childsDonate.findUnique({
        where: {
            id
        }
    });
...
async function ChildDonateList({ username }: { username: string }) {
    cookies();
    const donateList = await prisma.childsDonate.findMany({
        where: {
            childName: username,
        }, orderBy: {
            createAt: "desc"
                ...
                donateList.map((item, index: number) => (
                    <TableRow key={index}>
                        ...
                        <TableCell className="font-medium uppercase">
                            <DeleteButton type='child-donate' username={item?.id} />
                        </TableCell>
                    </TableRow>
                ))
            }
        ...
        <div className='flex flex-col'>
            ...
            <Table>
                ...
                <Suspense fallback={<h2 className=' text-center p-4'>Loading...</h2>} >
                    <ChildDonateList username={params.username} />
                </Suspense>
            </Table>
