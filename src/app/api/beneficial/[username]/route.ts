...
export const PATCH = async (request: Request, { params }: ParamsIProps) => {
    try {
        ...
        const { name, photoUrl, about, village, status, district, code, policeStation, occupation, phone, beneficialDonorId, nidFront, nidBack } = body;
        const result = await prisma.beneficial.update({
            where: { username },
            data: {
                name, photoUrl, about, village, status, district, code, policeStation, occupation, phone, beneficialDonorId, nidFront, nidBack
            }
        });
